...
def coerce_record_dates(data_type: str, record: dict[str, Any]) -> dict[str, Any]:
    ...
    # Legacy date columns must always be ISO format strings (parquet compatibility)
    legacy_date_columns = [
        "created_date",
        "updated_date",
        "purchase_date",
    ...
    for field in legacy_date_columns:
        if field not in new_record or new_record[field] is None:
            continue
        val = new_record[field]
        if isinstance(val, date) and not isinstance(val, datetime):
            new_record[field] = val.isoformat()
        elif isinstance(val, datetime):
            new_record[field] = val.date().isoformat()
        elif isinstance(val, pd.Timestamp):
            new_record[field] = val.date().isoformat()
        # If already str, leave as-is
...
def coerce_date_columns(df: pd.DataFrame, data_type: str | None = None) -> pd.DataFrame:
    ...
    # Legacy date columns must ALWAYS be stored as ISO format strings (parquet/PyArrow compatibility)
    legacy_date_columns = [
        "created_date",
        "updated_date",
        "purchase_date",
    ...
    for col_name in legacy_date_columns:
        if col_name in df.columns:
            ...
            df[col_name] = df[col_name].apply(_to_iso_string)
        ...
        if json_schema:
            ...
            for col_name, type_name in schema_def.items():
                if col_name not in df.columns:
                    continue
                # Keep legacy date columns as strings (already set above)
    ...
    # Fallback: generic coercion for date/time columns (skip legacy columns - they stay strings)
    for col in df.columns:
        if col in legacy_date_columns:
            continue
        name = str(col).lower()
...
def write_parquet_with_schema(
    ...
    # Ensure legacy date columns are strings before write (prevents "Expected bytes, got datetime.date" errors)
    legacy_date_columns = [
        "created_date",
        "updated_date",
        "purchase_date",
    ...
    for col_name in legacy_date_columns:
        if col_name in df.columns:
            ...
            df[col_name] = df[col_name].apply(_force_string)
            df[col_name] = df[col_name].astype("object")
    try:
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name == "list_data_types":
            ...
            if file_path.exists():
                try:
                    ...
                    for field in existing_schema:
                        col_name = field.name
                        if col_name in df_new.columns and isinstance(
                            field.type, pa.TimestampType
                        ):
                            # Ensure timezone-aware UTC timestamps to match schema
                            # Convert to datetime64[ns, UTC] dtype (not object)
                            df_new[col_name] = pd.to_datetime(
                                df_new[col_name], utc=True, errors="coerce"
                            )
        ...
        try:
            ...
            # Normalize legacy date columns to strings immediately after read
            legacy_date_columns = [
                "created_date",
                "updated_date",
                "purchase_date",
            ...
            for col_name in legacy_date_columns:
                if col_name in df.columns:
                    ...
                    df[col_name] = df[col_name].apply(_to_str)
                    df[col_name] = df[col_name].astype("object")
            ...
            # Ensure legacy date columns in updates are strings (coerce_record_dates may skip them)
            legacy_date_columns = [
                "created_date",
                "updated_date",
                "purchase_date",
            ...
            for key in list(updates.keys()):
                if key in legacy_date_columns and updates[key] is not None:
                    v = updates[key]
            ...
            for key, value in updates.items():
                ...
                elif schema_def and key in schema_def:
                    if (
                        key in legacy_date_columns
                        and value is not None
                        and not isinstance(value, str)
            ...
            # Update updated_date if column exists (use ISO string)
            if "updated_date" in df.columns:
                df.loc[mask, "updated_date"] = date.today().isoformat()
                df["updated_date"] = df["updated_date"].astype("object")
        ...
        try:
            ...
            if match_count == 0:
                ...
                # Set created_date/updated_date if columns exist (as ISO strings)
                ...
                # Apply updates (ensure legacy date columns are strings)
                legacy_date_columns = [
                    "created_date",
                    "updated_date",
                    "purchase_date",
            ...
            for record in embeddings_data:
                ...
                for key, value in record.items():
                    if key.endswith("_embedding") and value is not None:
                        if (
                            record_id_val
                            and record_id_val in existing_embeddings
                            and key in existing_embeddings[record_id_val]
                        ):
                            reused_count += 1
                        else:
                            generated_count += 1
